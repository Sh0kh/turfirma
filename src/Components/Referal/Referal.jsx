...
import { FaTelegram, FaInstagram, FaYoutube, FaTiktok, FaGlobe } from "react-icons/fa";
...
export default function Referal() {
  ...
  const platformStyles = {
    TELEGRAM: { color: "bg-[#229ED9]", icon: <FaTelegram className="text-white text-2xl" /> },
    INSTAGRAM: { color: "bg-gradient-to-r from-[#f58529] via-[#dd2a7b] to-[#8134af]", icon: <FaInstagram className="text-white text-2xl" /> },
    YOUTUBE: { color: "bg-[#FF0000]", icon: <FaYoutube className="text-white text-2xl" /> },
    TIKTOK: { color: "bg-black", icon: <FaTiktok className="text-white text-2xl" /> },
    OTHER: { color: "bg-gray-500", icon: <FaGlobe className="text-white text-2xl" /> },
  };
              ...
              >
                ...
                <div className="bg-white p-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-800 font-semibold">{i.clickCount}</span>
                  </div>
                </div>
              </div>
            );
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="text-left px-6 py-4 text-gray-500 font-medium text-sm uppercase tracking-wider">Nomi</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {referalData?.map((link) => (
                    <tr key={link.id} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-gray-800 font-medium">{link?.clickCount}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex gap-2">
                          <ReferalDelete id={link?.id} refresh={getAllReferal} />
                          <ReferalEdit data={link} refresh={getAllReferal} />
                        </div>
                      </td>
                    </tr>
                  ))}
